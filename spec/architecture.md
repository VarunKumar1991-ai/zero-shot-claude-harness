...
- **CSV parsing:** `csv-parse` (streaming parser) — chosen over loading the whole file into a naive split/regex parser because it correctly handles quoted fields, embedded commas/newlines, and streams for the ~100MB target size without holding the raw text twice in memory.
...
- Loading an entire uploaded CSV into a single synchronous JS string before parsing — always stream via `csv-parse` to stay within the ~100MB target without excess memory.
